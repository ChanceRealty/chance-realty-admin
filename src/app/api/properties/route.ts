// src/app/api/admin/properties/route.ts - FINAL FIX
...
export async function POST(request: Request) {
	try {
		...
		console.log('Creating property with data:', {
			...
			status: propertyData.status, // Log the status being sent
		...
		try {
			...
			// ✅ FIX: Convert status to integer ID that matches property_statuses table
			...
			if (propertyData.status) {
				...
				// If it's a string, look up the ID from property_statuses table
...
// ✅ FIXED GET function - Return status names by joining with property_statuses
